...
To change a function into a test function, add `#[test]` on the line before `fn`. When you run your tests with the
cargo `test command`, Rust builds a test runner binary that runs the annotated functions and reports on whether each
